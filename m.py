...
class Application(Frame):
	...
	def logo(self, master):
		photo = PhotoImage(file=resource_path("logo2.png"))
	...
	def Download_Location(self, master):
		...
		self.loc = ttk.Button(master, text="Change", command = self.callback).grid(row=0, column=2, pady=10)#grid(row=3, column=0, columnspan=3, padx=3)
